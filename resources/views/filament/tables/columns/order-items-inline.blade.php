    ...
    <div class="text-[11px] leading-tight text-gray-700">
        @foreach ($items as $item)
            ...
            <div class="mb-1.5 min-w-0">
                ...
                @if ($kitchenNote !== '')
                    <div class="mt-1 inline-block rounded px-1.5 py-0.5 text-[10px] font-medium" style="background:#fef08a;color:#713f12;">
                        Примітка: {{ $kitchenNote }}
                    </div>
                @endif
            </div>
        @endforeach
        ...
        @if ($orderKitchenNote !== '')
            <div class="mt-1.5 inline-block rounded px-2 py-1 text-[10px] font-semibold" style="background:#fde047;color:#713f12;">
                {{ $orderKitchenNote }}
            </div>
        @endif
    </div>
