    ...
    protected static function getCallcenterTotalsSchema(): array
    {
        ...
        $components[] = Placeholder::make('sidebar_receipt_buttons')
            ...
            ->content(function (?Order $record, Get $get): HtmlString {
                ...
                if ($record?->exists) {
                    $html .= '<button type="button" wire:click="mountAction(\'open_order_journal_sidebar\')" style="display:block;flex:1;min-width:160px;padding:10px 12px;border:1px solid #4b5563;border-radius:8px;background:#f9fafb;color:#374151;font-weight:700;cursor:pointer;text-align:center;">' . e(__('order.tabs.journal')) . '</button>';
                }

                $html .= '<button type="button" wire:click="mountAction(\'print_client_receipt_sidebar\')" style="display:block;flex:1;padding:10px 12px;border:1px solid #2563eb;border-radius:8px;background:#eff6ff;color:#1d4ed8;font-weight:700;cursor:pointer;text-align:center;">' . e(__('callcenter.order.print_client_receipt')) . '</button>'
