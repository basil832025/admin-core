...
return new class extends Migration
{
    public function up(): void
    {
        ...
        foreach (['rec_m1', 'rec_m2', 'rec_m3', 'rec_m4', 'rec_m5', 'del_m1', 'del_m2', 'del_m3', 'del_m4', 'del_m5'] as $token) {
            $body = str_replace(
                'rotate({{ '.$token.'|number_format(2, \'\.\', \'\') }}deg) translate(86px,0) rotate(-{{ '.$token.'|number_format(2, \'\.\', \'\') }}deg)',
                'rotate({{ ('.$token.' - 90)|number_format(2, \'\.\', \'\') }}deg) translate(86px,0) rotate(-{{ ('.$token.' - 90)|number_format(2, \'\.\', \'\') }}deg)',
                $body,
            );
        }
