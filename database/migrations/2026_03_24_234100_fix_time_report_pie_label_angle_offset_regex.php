...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $updated = preg_replace_callback(
            '/rotate\(\{\{\s*(rec_m[1-5]|del_m[1-5])\|number_format\(2, \'\\.\', \'\'\)\s*\}\}deg\) translate\(86px,0\) rotate\(-\{\{\s*\1\|number_format\(2, \'\\.\', \'\'\)\s*\}\}deg\)/',
            static function (array $matches): string {
                ...
                return "rotate({{ ({$token} - 90)|number_format(2, '.', '') }}deg) translate(86px,0) rotate(-{{ ({$token} - 90)|number_format(2, '.', '') }}deg)";
            },
            $body,
        );
